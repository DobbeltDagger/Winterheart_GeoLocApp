...
export class adminApp {
  ...
  ////////////////////////////////////////////////////////////////
  // handle input field value and sort
  inputHandler() {
    ...
    if (source.value.length < 3) {
      // do i have all rows rendered already?
      if (rowItems.length == this.data.length) { return }
      else { this.updateSearchResults(this.data); return }
    };
    ...
    for (let i = 0; i < this.data.length; i++) {
      const desc = this.data[i].Description.toLowerCase();
      const title = this.data[i].Title.toLowerCase();
      const images = this.data[i].Images;
      console.log("images:", images);
      const videos = this.data[i].Videos;
    ...
    // put matches in table!
    if (rowItems.length == matches.length) { return }
    else { this.updateSearchResults(matches); return }
  ...
  updateSearchResults(data) {
    ...
    // tblTest.id = "searchResultsTable";
    // tblwrapper.appendChild(tblTest);
    // return;
